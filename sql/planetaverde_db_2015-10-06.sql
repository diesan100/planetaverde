...
insert  into `area`(`id`,`parent`,`name`,`description`,`state`,`type`,`coords_in_parent`,`map_image`,`featured`) values (1,NULL,'World Map','This is the map of the world, this will be the parent container of all continents',1,0,'',3,0),(2,1,'South America','South America Area Descripcion',1,0,'111,171,115,191,118,211,119,246,125,234,135,217,144,202,150,188,154,179,142,174,136,164,124,158,112,160',3,0),(3,1,'Africa','African Continent Descripcion',1,0,'184,127,182,132,175,140,175,145,175,151,178,162,180,167,186,165,191,162,196,165,200,166,200,171,203,182,203,187,203,193,208,206,210,214,215,215,218,210,222,206,225,201,227,200,227,193,232,189,231,177,234,170,239,167,243,157,226,138,222,130,213,129,210,130,200,126,201,123,196,123,190,125',4,1),(5,1,'Asia','Asian Continent Descripcion',1,0,'250,114,245,128,254,138,264,141,264,148,268,156,272,152,274,147,281,142,289,149,288,164,298,176,305,179,316,179,317,171,322,159,319,140,319,132,322,128,331,128,334,122,338,116,340,111,337,99,338,90,349,90,353,100,360,91,361,88,369,84,376,79,383,74,379,68,362,63,340,59,335,52,313,51,302,44,288,41,268,53,249,68,242,76,242,93,243,105',12,1),(6,1,'Antartica','Antartica continent',1,1,'149,255,178,250,217,258,183,264',NULL,0),(7,1,'Europe','europe',1,0,'111,171, 115,191, 118,211, 119,246, 125,234, 135,217, 144,202, 150,188, 154,179, 142,174, 136,164, 124,158, 112,160 ',3,0),(8,3,'Madagascar','An island country in the Indian Ocean, Madagascar is located off the southeastern coast of Africa. The nation comprises the island of Madagascar (the 4th largest island of the world), as well as numerous smaller peripheral islands.',1,1,'249,182,247,190,241,199,247,205,251,209,255,210,258,205,264,198,265,191,263,181,266,179,266,171,262,170',7,1);
...
CREATE TABLE `custom_trip` (
  ...
  `flight_arrival` varchar(200) DEFAULT NULL COMMENT 'if null, use base trip''s flights',
  `flight_return` varchar(200) DEFAULT NULL,
  `state` tinyint(4) DEFAULT '0',
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `user_id` (`user_id`),
  CONSTRAINT `custom_trip_ibfk_1` FOREIGN KEY (`user_id`) REFERENCES `user` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
/*Table structure for table `custom_trip_item` */

DROP TABLE IF EXISTS `custom_trip_item`;

CREATE TABLE `custom_trip_item` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `trip_id` int(11) NOT NULL COMMENT 'Custom trip ID',
  `day_order` smallint(6) NOT NULL,
  `lodge_id` int(11) NOT NULL,
  `nights` smallint(6) NOT NULL DEFAULT '1',
  `note` varchar(300) DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `trip_id` (`trip_id`),
  CONSTRAINT `custom_trip_item_ibfk_1` FOREIGN KEY (`trip_id`) REFERENCES `custom_trip` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

/*Data for the table `custom_trip_item` */
...
CREATE TABLE `feedback` (
  ...
  `lotype` tinyint(4) NOT NULL DEFAULT '1' COMMENT '1: lodge, 2: group-trip',
  `loid` int(11) NOT NULL COMMENT 'id of lodge or gt',
...
CREATE TABLE `group_trip` (
  ...
  `trip_type` char(1) DEFAULT NULL COMMENT 'C: comfort',
  `title` varchar(50) NOT NULL,
  `subtitle` varchar(150) NOT NULL,
  `description` text NOT NULL,
  `itinerary` text,
  `dateprice` text,
  `poll_rate` decimal(2,2) DEFAULT NULL COMMENT 'feedback ratio',
  `image` int(11) NOT NULL,
  `flight_arrival` varchar(200) DEFAULT NULL,
  `flight_return` varchar(200) DEFAULT NULL,
...
insert  into `group_trip`(`id`,`area_id`,`trip_type`,`title`,`subtitle`,`description`,`itinerary`,`dateprice`,`poll_rate`,`image`,`flight_arrival`,`flight_return`,`created_at`,`updated_at`,`state`) values (1,8,'C','Group-Trip 1','Travel in Madagascar','alskje sjdl jo selsfe jlaesf osdfkls e','itinerary alsefi sdf ksjeli s','date and price ',NULL,8,'','',NULL,'2015-10-02 07:18:34',1);

/*Table structure for table `group_trip_item` */

DROP TABLE IF EXISTS `group_trip_item`;

CREATE TABLE `group_trip_item` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `trip_id` int(11) NOT NULL COMMENT 'Group-trip ID',
  `day_order` smallint(5) unsigned DEFAULT NULL,
  `lodge_id` int(11) NOT NULL,
  `nights` smallint(6) NOT NULL DEFAULT '1' COMMENT 'staying nights at lodge',
  `note` varchar(300) DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `trip_id` (`trip_id`),
  CONSTRAINT `group_trip_item_ibfk_1` FOREIGN KEY (`trip_id`) REFERENCES `group_trip` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=19 DEFAULT CHARSET=latin1;

/*Data for the table `group_trip_item` */

insert  into `group_trip_item`(`id`,`trip_id`,`day_order`,`lodge_id`,`nights`,`note`) values (12,1,NULL,2,3,NULL),(16,1,NULL,1,2,NULL),(17,1,NULL,2,4,NULL),(18,1,NULL,1,1,NULL);
...
insert  into `lodge`(`id`,`area_id`,`name`,`description`,`state`,`notes`,`img`,`poll_rate`) values (1,8,'Lifes Hotel','Lifes Hotel in Madagascar',1,'Notes*',11,NULL),(2,8,'Lotus Hood','Hood in the lotus flowers',1,'Lotus Hood',12,NULL);
...
CREATE TABLE `wishlist` (
  ...
  PRIMARY KEY (`id`),
  KEY `user_id` (`user_id`),
  CONSTRAINT `wishlist_ibfk_1` FOREIGN KEY (`user_id`) REFERENCES `user` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
